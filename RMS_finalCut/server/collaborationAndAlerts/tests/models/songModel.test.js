...
describe('Song Model Test Suite', function () {
  ...
  describe('Post Hooks and Aggregates', function() {
    it('should trigger post-save hook to update artist totals after creating a song', async function() {
      // Create a new song
      const song = await Song.create({
        artistId: testArtist._id,
        artistName: testArtist.fullName,
        songId: 'post-hook-song-123',
        songName: 'Post Hook Song',
        totalRoyalty: 200,
        totalStreams: 3000
      });
  
      // Verify that the artist totals were updated after save
      const updatedArtist = await Artist.findById(testArtist._id);
      expect(updatedArtist.fullRoyalty).to.equal(200);
      expect(updatedArtist.totalStreams).to.equal(3000);
    });
  
    it('should trigger post-update hook to update artist totals after updating a song', async function() {
      // First create a song
      const song = await Song.create({
        artistId: testArtist._id,
        artistName: testArtist.fullName,
        songId: 'song-update-hook',
        songName: 'Song Update Hook',
        totalRoyalty: 50,
        totalStreams: 500
      });
      ...
      await Song.findOneAndUpdate(
        { _id: song._id },
        { totalRoyalty: 150, totalStreams: 1500 },
        { new: true }
      );
  
      // Verify that the artist totals were updated after update
      const updatedArtist = await Artist.findById(testArtist._id);
      expect(updatedArtist.fullRoyalty).to.equal(150);
      expect(updatedArtist.totalStreams).to.equal(1500);
    });
